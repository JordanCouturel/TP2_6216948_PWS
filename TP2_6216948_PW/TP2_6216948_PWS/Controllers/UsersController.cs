...
namespace TP2_6216948_PWS.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        //        SymmetricSecurityKey key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes("clesupervraimentbeaucouplonguequiferarienbugger"));
        ...
        public async Task<IActionResult> Register(RegisterDTO model)
        {
            try
            {
                ...
                var (status, message) = await _authService.Registeration(model, UsersRoles.User);
                if (status == 0)
                {
